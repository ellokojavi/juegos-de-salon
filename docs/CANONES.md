...
- **Cada partida deja una señal de uso para el panel del dueño** (`assets/js/transport/stats.js`, D-44). El transporte apunta las salas solo; los modos sin red llaman `trackStart({ game, mode, players })` al empezar (no al retomar). Es un `fetch` por REST, sin SDK, mejor esfuerzo: nunca se espera ni se muestra. De los modos sin red no sale ningún nombre, y nunca sale un secreto, el chat, quién ganó ni una IP.
