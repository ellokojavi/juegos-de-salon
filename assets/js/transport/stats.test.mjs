...
const fp0 = fingerprint({ loc: null, nav: null, doc: null, now: new Date(2026, 8, 11, 21, 5) });
...
const fp = { env: 'prod', v: '0.20.0', tz: 'America__Santiago', lang: 'es-CL', hour: 21 };

// Contadores por inicio de partida
assert.deepEqual(startChanges(fp, { game: 'toque-y-fama', mode: 'cpu', players: 1 }), {
  'origin/America__Santiago': INC, 'lang/es-CL': INC, 'hour/21': INC, 'local/toque-y-fama/cpu/1': INC,
});
...
assert.deepEqual(Object.keys(online).sort(), ['hour/21', 'lang/es-CL', 'origin/America__Santiago']);
